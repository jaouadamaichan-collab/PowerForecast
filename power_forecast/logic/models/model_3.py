...
def add_rolling_zscore(df: pd.DataFrame, target: str, window: int = 168) -> pd.DataFrame:
    ...
    Args:
        ...
        window (int): Taille de la fenêtre glissante (défaut : 168h = 7 jours).
...
# ================================================================= #
# FEATURE : PRIX DU GAZ EUROPÉEN                                    #
# ================================================================= #
# Le prix du gaz (mensuel) est la feature manquante pour expliquer  #
# les folds de crise énergétique 2021-2022 où FRA explose.          #
# On resamble le pas mensuel vers le pas horaire par forward fill.   #
# Les mois sans données (avant 2017-01-01) sont remplis par bfill.  #

def add_gas_price(df: pd.DataFrame, filepath: str = "raw_data/PNGASEUUSDM.csv") -> pd.DataFrame:
    """
    Ajoute le prix du gaz européen (PNGASEUUSDM) au DataFrame horaire.

    Le CSV source est au pas mensuel (2017-01 à 2025-12). Il est
    rééchantillonné au pas horaire par forward fill, puis aligné sur
    l'index exact du DataFrame principal.

    Les heures antérieures à 2017-01-01 (pas de données gaz) sont
    remplies par backward fill (premier prix connu propagé en arrière).
    C'est une approximation acceptable car ces périodes correspondent
    aux folds hors-crise.

    Args:
        df (pd.DataFrame): DataFrame principal au pas horaire avec index DatetimeTZ.
        filepath (str): Chemin vers le CSV des prix du gaz.

    Returns:
        pd.DataFrame: df avec une colonne supplémentaire 'gas_price_eu'.
    """
    df_gaz = pd.read_csv(filepath, parse_dates=["observation_date"])
    df_gaz = df_gaz.set_index("observation_date")
    df_gaz.index = df_gaz.index.tz_localize("UTC")

    # Resampler mensuel → horaire par forward fill
    df_gaz_hourly = df_gaz.resample("h").ffill()

    # Aligner sur l'index exact du DataFrame principal
    df_gaz_hourly = df_gaz_hourly.reindex(df.index, method="ffill")

    # Remplir les NaN avant 2017-01-01
    n_nan_before = df_gaz_hourly.isna().sum().sum()
    df_gaz_hourly = df_gaz_hourly.bfill()
    n_nan_after = df_gaz_hourly.isna().sum().sum()
    print(f"Prix du gaz — NaN avant bfill : {n_nan_before} | après : {n_nan_after}")

    df = df.copy()
    df["gas_price_eu"] = df_gaz_hourly["PNGASEUUSDM"]

    print(f"gas_price_eu ajoutée — mean: {df['gas_price_eu'].mean():.2f} "
          f"| min: {df['gas_price_eu'].min():.2f} "
          f"| max: {df['gas_price_eu'].max():.2f}")
    return df


df = add_gas_price(df, filepath="raw_data/PNGASEUUSDM.csv")
...
# Forcer FRA_zscore si éliminée par Lasso
...
# Forcer gas_price_eu si éliminée par Lasso
if 'gas_price_eu' not in selected_features:
    selected_features.append('gas_price_eu')
    print("'gas_price_eu' forcée — feature structurellement liée aux folds de crise.")
...
def clean_target_outliers(df: pd.DataFrame, target: str,
    ...
    """
    Clippe les valeurs aberrantes de la target aux percentiles spécifiés.

    Args:
        df (pd.DataFrame): DataFrame avec la colonne target.
        target (str): Nom de la colonne cible.
        lower_pct (float): Percentile bas (défaut : 2%).
        upper_pct (float): Percentile haut (défaut : 98%).

    Returns:
        pd.DataFrame: DataFrame avec target clippée.
    """
    df = df.copy()
